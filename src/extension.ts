...
export function activate(context: vscode.ExtensionContext) {
  ...
  const disposable = vscode.commands.registerCommand(
    ...
    () => {
      ...
      const languageComments = config.get<{ [key: string]: string }>(
        "languageComments",
        {}
      );
      // 言語IDに対応するコメントプレフィクスを取得
      const commentPrefix = languageComments[languageId];
      if (!commentPrefix) {
        vscode.window.showWarningMessage(
          `No comment prefix defined for language: ${languageId}. ` +
            `You can add it in settings under "commentput.languageComments".`
        );
        return;
      }
